...
| `app/admin` | Invite mint. Invite-gated, admin role only. |
| `app/api/*` | Vertical-slice API routes (auth, packs, composer, starters, jobs, pack test-grid / retrain, per-user Venice settings) |
...
- Put your email in `ADMIN_EMAILS` so the first redeemed account can open **Invites** (`/admin/invites`). Stub already treats `jai@cast.review` as admin. **Settings** (`/app/settings`) is available to every signed-in user.
...
| `ADMIN_EMAILS` | `jai@cast.review` | Optional. Unset on cookie-only review: every invite redeem is admin so **Invites** appears. Stub always includes `jai@cast.review`. **Settings** does not require admin. |
...
Click-through: `/` → invite (any email) + code **`castreview`** (pre-filled on stub) → age **I confirm I am 18+.** → `/app/characters` → studio rail **Settings** → `/app/settings` (Connect Venice). Alias: `/studio/settings` → `/app/settings`. Unauthenticated `/app/settings` redirects to `/invite`. Admin-only invite tools stay at `/admin/invites` (signed-out `/admin` redirects to `/invite`; non-admin sessions redirect to `/app`).

Connect Venice lives at **`/app/settings`** for every signed-in account. `/admin` is invite tools only.
...
- **Connect Venice (Settings):** every signed-in user opens **Settings** (`/app/settings`). Paste an API key from [venice.ai/settings/api](https://venice.ai/settings/api), **Save**. Cast validates with `GET {VENICE_API_BASE_URL}/api_keys/rate_limits` (Bearer; Inference Only keys are allowed). `GET /models` is public and is not used to verify a key. A valid key is stored **encrypted per user** (AES-256-GCM using `SESSION_SECRET`, bound to that user id) in Postgres `user_secrets`. The UI then shows **Connected** and `••••last4` — the full key is never returned to the browser, RSC payload, or logs. **Disconnect** clears that user's stored key and stops Cast from using a deploy `VENICE_API_KEY` for *that user* until they save a new one. Other accounts are unchanged.
- Flip Generate from stub → Venice: `PROVIDER_MODE=live`, `GENERATE_STILL_PROVIDER=venice` (already the default), and a connected key for **that user** (Settings **or**, if they have never saved/disconnected, `VENICE_API_KEY`). `PROVIDER_MODE` is `stub` | `live` (not `venice`). Stub stays the default for preview deploys — saving a key in stub still validates it, but `generateStill` stays on the stub adapter until you flip live.
- Where the worker reads the key: `resolveVeniceApiKey(job.userId)` prefers that user's Settings row, then `VENICE_API_KEY` if they have never saved. Disconnect opts that user out of the env fallback. No redeploy is required after **Save** / **Disconnect** when `DATABASE_URL` is set (Next app and `pnpm worker` share Postgres). Cookie-only stub preview (`PROVIDER_MODE=stub`, no `DATABASE_URL`) has no Postgres: Save writes an AES-256-GCM overlay into a signed httpOnly `cast_venice` cookie (HMAC via `SESSION_SECRET`) keyed by that user's id and email hash. Disconnect expires that cookie. Stub invite redeem reuses a stable user id for the same email, so **Connected** + `••••last4` survive sign-out → redeem again. Generate remains stub until you flip live.
- Vercel env path: you can still set `VENICE_API_KEY` on the project (Preview/Production) without using Settings. That needs a deploy/env sync for the Next app **and** the worker to see a new env value. Settings is the no-redeploy, per-account path. Rotating `SESSION_SECRET` invalidates encrypted rows — each user saves their Venice key again. Unset `VENICE_API_KEY` in Vercel if you want the deploy secret gone as well as Disconnect.
...
- Jobs list/get (and enqueue 202 bodies) include `ageSeconds`, `attemptCount`, `lastErrorCode`, and `lastError` (user-safe copy — never a prompt or provider payload). The Jobs page groups **Still / Train / Test grid / Starter** and filters by kind or status (`?kind=` / `?status=`). Create uses `lastError` when Generate fails. Queued/generating stills show queue-wait or “Generating this still…” copy. `POST /api/jobs/:id/cancel` cancels queued or generating stills (and starters); Train & lock is refused with a user-safe reason. Canceled is not a failure.
