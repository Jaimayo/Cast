...
export function PackStatusPanel(props: { pack: Pack; refCount: number }) {
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-8 md:px-8">
      <p className="text-[11px] tracking-[0.16em] text-muted-foreground uppercase">Character Pack</p>
      <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-heading text-4xl">{props.pack.name}</h1>
      ...
      {status === "training" ? (
        <p className="mt-4 text-sm text-success">Training Soul ID…</p>
      ) : null}
      {adapterReady ? (
        <p className="mt-3 text-sm text-success">Identity adapter saved. Generate uses this Soul ID on stills.</p>
      ) : locked ? (
