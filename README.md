...
- Where the worker reads the key: `resolveVeniceApiKey(job.userId)` prefers that user's Settings row, then `VENICE_API_KEY` if they have never saved. Disconnect opts that user out of the env fallback. No redeploy is required after **Save** / **Disconnect** when `DATABASE_URL` is set (Next app and `pnpm worker` share Postgres). Cookie-only stub preview (`PROVIDER_MODE=stub`, no `DATABASE_URL`) has no Postgres: Save writes an AES-256-GCM overlay into a signed httpOnly `cast_venice` cookie (HMAC via `SESSION_SECRET`) keyed by that user's id and email hash. Stub invite redeem reuses a stable user id for the same email, so **Connected** + `••••last4` survive sign-out → redeem again. Generate remains stub until you flip live.
