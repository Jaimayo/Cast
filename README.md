...
- **Connect Venice (Settings):** signed-in admins open **Settings** (`/admin`). Paste an API key from [venice.ai/settings/api](https://venice.ai/settings/api), **Save key**. Cast validates with `GET {VENICE_API_BASE_URL}/models?type=image` (Bearer) and stores the key **encrypted** (AES-256-GCM using `SESSION_SECRET`) in Postgres `operator_secrets`. The UI then shows **Connected** and `••••last4` — the full key is never returned to the browser, RSC payload, or logs. **Disconnect** clears the stored key and stops Cast from using a deploy `VENICE_API_KEY` until you save a new one.
